...
/**
 * @brief Get the Ai's Move
 *  ** I know it's not really AI but if you are interested look at minimax algorithm **
 *  ** There are loads of examples of it for tic tac toe **
...
int getAiMove()
{
    ...
    for (int i = 0; i < sizeof(squares); i++) {
        if(squares[i] == ' ') freeSquares.push_back(i);
    }
    int index = std::rand() % freeSquares.size();
